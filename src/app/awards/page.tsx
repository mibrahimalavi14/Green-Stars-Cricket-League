...
async function AwardsPage() {
  ...
  const seasonsAwards = seasons.map(season => {
    ...
    const agg: Record<string, { playerId: string; name: string; team?: any; runs: number; ballsFaced: number; sixes: number; wickets: number; runsConceded: number; catches: number; matchesPlayed: number; maidens: number }> = {}
    for (const pm of spm) {
      const pid = pm.playerId
      if (!agg[pid]) agg[pid] = { playerId: pid, name: pm.player.name, team: pm.player.team, runs: 0, ballsFaced: 0, sixes: 0, wickets: 0, runsConceded: 0, catches: 0, matchesPlayed: 0, maidens: 0 }
      const a = agg[pid]
    ...
    const mostMaidens = [...players].sort((a, b) => b.maidens - a.maidens)[0]
    ...
    return { season, winner, topScorer, topWicketTaker, mostSixes, bestStrikeRate, bestAllRounder, mostCatches, mostMaidens, potmList }
  })
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      {seasonsAwards.map(({ season, winner, topScorer, topWicketTaker, mostSixes, bestStrikeRate, bestAllRounder, mostCatches, mostMaidens, potmList }) => {
        ...
        <div key={season.id} className="mb-12">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <AwardCard icon={Medal} label="Most Maidens" value={mostMaidens ? String(mostMaidens.maidens) : "-"} stat="Maidens" name={mostMaidens?.name || "N/A"} team={mostMaidens?.team?.shortName} color="blue" />
