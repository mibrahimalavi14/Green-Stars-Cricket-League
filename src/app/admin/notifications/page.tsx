  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <form onSubmit={create} className="mb-10 space-y-4 rounded-lg border border-[var(--border)] bg-[var(--card)] p-6">
        ...
        <button type="submit" className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-90">Send Notification</button>
      </form>
        ...
        {notifs.length === 0 ? (
          ...
          notifs.map((n) => (
            <div key={n.id} className={`flex items-center justify-between rounded-lg border p-4 ${n.read ? "border-[var(--border)]" : "border-yellow-500/30 bg-yellow-500/5"}`}>
              ...
              <button onClick={() => remove(n.id)} className="ml-4 shrink-0 rounded bg-red-500/20 px-3 py-1 text-xs font-semibold text-red-700 transition-opacity hover:opacity-80 dark:text-red-400">Delete</button>
            </div>
          ))
        )}
      </div>
