...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Auto-generate season quiz when the season's last match completes
    let quizAutoGenerated = false
    try {
      const { maybeAutoGenerateSeasonQuiz } = await import("@/lib/season-quiz")
      const quizResult = await maybeAutoGenerateSeasonQuiz(match.seasonId)
      if (quizResult.autoGenerated) {
        quizAutoGenerated = true
        logAudit({
          action: "season_quiz_auto_generated",
          entity: "season",
          entityId: match.seasonId,
          details: JSON.stringify({ count: quizResult.count }),
          ip,
        })
      }
    } catch (quizErr) {
    ...
    // Auto-generate season awards when the season's last match completes
    try {
      const { maybeAutoGenerateSeasonAwards } = await import("@/lib/season-awards")
      const awardsResult = await maybeAutoGenerateSeasonAwards(match.seasonId)
      if (awardsResult.autoGenerated) {
        logAudit({
          action: "season_awards_auto_generated",
          entity: "season",
          entityId: match.seasonId,
          details: JSON.stringify({ count: awardsResult.count }),
          ip,
        })
      }
    } catch (awardsErr) {
      console.error("Season awards auto-generation failed:", awardsErr)
    }
    ...
    // Auto push notifications: match result + POTM voting
    try {
      const { sendPushNotification } = await import("@/lib/push")
      await sendPushNotification({
        title: "Match Complete!",
        body: result,
        link: `/matches/${matchId}/potm`,
      })
      if (quizAutoGenerated) {
        await sendPushNotification({
          title: "Season Quiz is live!",
          body: "Test your knowledge on /quiz before it closes.",
          link: "/quiz",
        })
      }
    } catch (pushErr) {
