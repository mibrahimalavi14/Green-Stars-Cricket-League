...
export async function PATCH(req: Request) {
  ...
  const existingMatch = await prisma.match.findUnique({ where: { id }, select: { seasonId: true } })
  const { assertSeasonUnlocked } = await import("@/lib/season-guard")
  const lockErr = await assertSeasonUnlocked(existingMatch?.seasonId)
  if (lockErr) return NextResponse.json({ error: lockErr }, { status: 423 })
  ...
  if (data.status === "completed") {
    ...
    if (fullMatch) {
      ...
      await recalcPlayerStats(fullMatch.seasonId)
      try {
      ...
      try {
        ...
        const awardsResult = await maybeAutoGenerateSeasonAwards(fullMatch.seasonId)
        if (awardsResult.autoGenerated) {
          logAudit({ action: "season_awards_auto_generated", entity: "season", entityId: fullMatch.seasonId, details: JSON.stringify({ count: awardsResult.count }), ip })
        }
      } catch (awardsErr) {
