...
function PredictionsInner({
  ...
  useEffect(() => {
    if (session?.user?.email) {
      const existing = predictions.find(p => p.email === session!.user!.email)
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!session?.user?.name || !session?.user?.email || !selectedTeam) return
    if (locked) return
    setSubmitting(true)
    setMessage("")
    try {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      ...
      {!locked && !session && (
        <div className="mb-10 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8 text-center">
          <h2 className="mb-2 text-lg font-semibold">Continue with Google</h2>
          <p className="mb-6 text-sm text-[var(--muted-foreground)]">Sign in with your Google account to cast your vote</p>
          <button
            onClick={() => signIn("google", { callbackUrl: "/predictions", redirect: true })}
            className="inline-flex items-center gap-3 rounded-lg border border-[var(--border)] bg-white px-8 py-3 font-medium text-gray-900 transition-all hover:bg-gray-50 dark:bg-gray-900 dark:text-white dark:hover:bg-gray-800"
          >
      ...
      {!locked && session && (
        <form onSubmit={handleSubmit} className="mb-10 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6">
          <div className="mb-6 flex items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--background)] px-4 py-3">
            {session.user?.image ? (
              <img src={session.user.image} alt="" className="h-8 w-8 rounded-full" />
            ) : (
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--muted)] text-sm font-bold">
            ...
            <span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-400">Signed in</span>
          </div>

          <div className="mb-6">
            <label className="mb-3 block text-sm font-medium">Select Your Predicted Winner</label>
            {hasVoted ? (
              <p className="text-sm text-green-600">You have already cast your vote for this season!</p>
            ) : (
            ...
            <div key={team.id} className="flex items-center gap-4 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold">{team.votes}</span>
                </div>
                <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-[var(--muted)]">
                  <div
                    className="h-full rounded-full transition-all"
                    style={{
                      width: `${teamsWithVotes[0].votes > 0 ? (team.votes / teamsWithVotes[0].votes) * 100 : 0}%`,
      ...
      <div>
        <h2 className="mb-4 text-xl font-semibold">
          Recent Votes ({predictions.length})
        </h2>
