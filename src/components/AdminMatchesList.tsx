...
export function AdminMatchesList({ matches }: { matches: Match[] }) {
  ...
  async function submitScore(id: string, m: Match) {
    ...
    const playersData = allPlayers
      ...
      .filter(p => s(p.id, "runs") || s(p.id, "wkts") || s(p.id, "ct"))
      .map(p => ({
        playerId: p.id,
        teamId: p.teamId,
          ...
          <tbody>
            {players.map(p => (
              <tr key={p.id} className="border-b border-[var(--border)] hover:bg-[var(--background)]">
                ...
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "runs")} onChange={e => set(p.id, "runs", e.target.value)} className="w-12 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "bf")} onChange={e => set(p.id, "bf", e.target.value)} className="w-12 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "4s")} onChange={e => set(p.id, "4s", e.target.value)} className="w-10 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "6s")} onChange={e => set(p.id, "6s", e.target.value)} className="w-10 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "1s")} onChange={e => set(p.id, "1s", e.target.value)} className="w-10 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "2s")} onChange={e => set(p.id, "2s", e.target.value)} className="w-10 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1 text-center"><input type="checkbox" checked={s(p.id, "out") === "1"} onChange={() => toggleOut(p.id)} className="h-3 w-3" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "wkts")} onChange={e => set(p.id, "wkts", e.target.value)} className="w-10 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "br")} onChange={e => set(p.id, "br", e.target.value)} className="w-12 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 px-1"><input type="number" min="0" value={s(p.id, "bb")} onChange={e => set(p.id, "bb", e.target.value)} className="w-12 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
                <td className="py-1 pl-1"><input type="number" min="0" value={s(p.id, "ct")} onChange={e => set(p.id, "ct", e.target.value)} className="w-10 rounded border border-[var(--border)] bg-[var(--card)] px-1 py-0.5 text-center" /></td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          {scoring === m.id && (
            <div className="mt-3 space-y-3 border-t border-[var(--border)] pt-3">
              ...
              {team1Players.length > 0 && (
                <div>
                  <p className="mb-1 text-sm font-semibold" style={{ color: m.team1.color }}>{m.team1.shortName}</p>
                  {PlayerTable(team1Players, m.team1.color, m.team1.shortName)}
                </div>
              )}
              {team2Players.length > 0 && (
                <div>
                  <p className="mb-1 mt-3 text-sm font-semibold" style={{ color: m.team2.color }}>{m.team2.shortName}</p>
